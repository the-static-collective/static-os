...
# Python 3.11's setuptools vendor of jaraco.context imports backports.tarfile.
# Bundle its namespace explicitly: a build is not usable until frozen self-test runs.
...
"$BUILD/venv/bin/pyinstaller" --noconfirm --onedir --name static-workbench-desktop \
  --collect-all static_workbench --collect-all uvicorn --collect-all backports \
