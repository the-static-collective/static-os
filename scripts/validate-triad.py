...
def validate(value, candidate_manifest=None):
    if not isinstance(candidate_manifest, dict):
        raise ValueError("current Genesis candidate manifest required")
    house = candidate_manifest.get("house")
    elf = candidate_manifest.get("elf")
    if (not isinstance(house, dict) or not isinstance(elf, dict)
            or not isinstance(house.get("commit"), str) or not SHA.fullmatch(house["commit"])
            or elf.get("source_commit") != house["commit"]
            or elf.get("automatic") is not False or elf.get("authority") != "none"):
        raise ValueError("ELF must be manual, zero-authority and match pinned HOUSE")
    ...
    if sources[2]["commit"] != house["commit"]:
        raise ValueError("Triad Workbench pin must match current candidate HOUSE")
...
def main(argv=None):
    ...
    try:
        validate(json.loads(Path(args[0]).read_text(encoding="utf-8")),
                 json.loads((Path(__file__).resolve().parents[1] / "manifest" / "genesis-001.json").read_text(encoding="utf-8")))
